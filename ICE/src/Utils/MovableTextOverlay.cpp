...
#include "Utils/MovableTextOverlay.h"

#define LIFETIME 2
#define VERTICAL_VELOCITY 0.1
...
void MovableTextOverlay::initialize(const Ogre::String & caption, const Ogre::MovableObject *mov, MovableTextOverlayAttributes *attrs)
{
	mpMov = mov;
	mAttrs = attrs;
	if (caption == "")
...
void MovableTextOverlay::enable()
{
	if (mEnabled == true)
		return;
...
void MovableTextOverlay::disable()
{
	if (mEnabled == false)
		return;
...
void MovableTextOverlay::update(Real timeSincelastFrame)
{
 ...
 //
	...
	mYOffset += VERTICAL_VELOCITY * timeSincelastFrame;
	max_y += mYOffset;
